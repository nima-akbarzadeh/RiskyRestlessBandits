...
class deadlineSchedulingMultipleArmsEnv(gym.Env, ABC):
    ...
    def _createActionTable(self):
        """function that creates a mapping of actions to take. Will be mapped with the action
    ...
    def _calReward(self, action):
        ...
        if self.actionTable is not None:
    ...
    def step(self, action):
        """ Standard Gym function for taking an action. Supplies nextstate, reward, and episode
        termination signal. """
    ...
    def _setTheArms(self):
        ...
        for i in range(self.numArms):
            self.envs[i] = deadlineSchedulingEnv(seed=self.envSeeds[i], numEpisodes=1,
                                                 episodeLimit=self.episodeLimit,
                                                 maxDeadline=self.maxDeadline, maxLoad=self.maxLoad,
                                                 newJobProb=self.newJobProb,
                                                 processingCost=self.processingCost, train=False,
                                                 batchSize=self.batchSize, noiseVar=self.noiseVar)
