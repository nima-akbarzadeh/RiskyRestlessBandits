...
# this package throws errors. it's normal. requires python 3.6.
...
class deadlineSchedulingEnv(gym.Env):
    ...
    def __init__(self, seed, numEpisodes, episodeLimit, maxDeadline, maxLoad, newJobProb,
        ...
        # first: laxity T (D in the paper). Second: load B. Third: deadline d. initalized to all ones
        self.arm = {0: [1, 1, 1]}
    ...
    def _calReward(self, action, state):
        ...
        if action == 1:
            ...
            noise = self.noiseVector[
                self._findStateIndex(currentState) + np.shape(self.stateArray)[0]]
            if (currentState[1] == 0) and (currentState[0] == 0):
                reward = 0
            elif (currentState[1] >= 0) and (currentState[0] > 1):
                reward = 0
    ...
    def reset(self):
        ...
        if self.miniBatchCounter % self.batchSize == 0:
            ...
            self.jobList = self.G.choice([1, 0], p=[self.newJobProb, 1 - self.newJobProb],
                                         size=self.episodeLimit)
