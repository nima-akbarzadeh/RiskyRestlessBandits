...
linestyle s =['solid', 'dotted', 'dashed', 'dashdot', (0, (3 ,1 ,1 ,1 ,1 ,1))]
...
modelDir = savedDirector y + \
    (f'seed_{50}_lr_0.001_batchSize_{5}_trainedNumEpisodes_{EPISODESTRAINED}/trained_model.pt')
...
for state in statesToTest:
    ...
    for currentAC in currentActivationCost:
        ...
        for i in range(NORUNS):
            ...
            Q_active += BET A* *(0 ) *(reward - currentAC)
            ...
            for x in range(1, TIMESTEPS):
                ...
                Q_active += BET A* *(x ) *(reward - currentA C *action)
            ...
            for g in range(1, TIMESTEPS):
                ...
                Q_passive += BET A* *(g ) *(reward - currentA C *action)
...
D_final = []
numOfStatesToTest = 5
states = np.random.randint(1 ,MAXWAI T +1, size=numOfStatesToTest)
...
savedDirectory = \
    (f'../trainResults/neurwin/recovering_bandits_env/recovery_function_{selectedActivationFunction}/')
modelDir = savedDirectory + (
    f'seed_{50}_lr_0.001_batchSize_{5}_trainedNumEpisodes_{EPISODESTRAINED}/trained_model.pt')
...
for x in range(len(states)):
    axes[1].plot(currentActivationCost, D_final[x], marker='.', label=f's = {states[x]}.',
                 linewidth=LINEWIDTH, linestyle=linestyles[x])
    ...
    for cost in currentActivationCost:
        ...
        for i in range(NORUNS):
            ...
            env = sizeAwareIndexEnv(numEpisodes=1, HOLDINGCOST=1, seed=seed[i], Training=False,
                                    r1=8400, r2=33600, q=goodStateProb, case=1, classVal=armClass,
                                    load=state[0], noiseVar=0.0,
                                    maxLoad=MAXLOAD, batchSize=5, episodeLimit=TIMESTEPS,
                                    fixedSizeMDP=False)
...
for q in range(np.shape(statesToTest)[0]):
    axes[2].plot(currentActivationCost, D_final[q], marker='.', label=f's = {statesToTest[q]}',
                 linewidth=LINEWIDTH, linestyle=linestyles[q])
