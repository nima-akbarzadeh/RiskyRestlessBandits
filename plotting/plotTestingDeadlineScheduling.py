...
def plotWolp(ARMS, SCHEDULE, INTERVAL):
    ...
    for i in range(RUNS):
        if ARMS == 100:
            wolpFileName = (
                f'../testResults/deadline_env/wolpResults_arms_{ARMS}_batchSize_{BATCHSIZE}_run_{i}_timeLimit_{TIMELIMIT}_schedule_{SCHEDULE}.csv')
        else:
            wolpFileName = (
                f'../wolpertinger_ddpg/testResults/deadline_ddpg_results_{ARMS}_arms_choose_{SCHEDULE}_run_{i}.csv')
...
def plotReinforce(ARMS, SCHEDULE, INTERVAL):
    ...
    for i in range(RUNS):
        reinforceFileName = (
            f'../testResults/deadline_env/reinforceResults_arms_{ARMS}_batchSize_{BATCHSIZE}\
...
def plotAQL(ARMS, SCHEDULE, INTERVAL):
    ...
    for i in range(RUNS):
        aqlFileName = (
            f'../testResults/deadline_env/aqlResults_arms_{ARMS}_run_{i}_schedule_{SCHEDULE}.csv')
...
def plotQWIC(ARMS, SCHEDULE, INTERVAL):
    ...
    for i in range(RUNS):
        qLearningFileName = (
            f'../testResults/deadline_env/qLearningResults_arms_{ARMS}_run_{i}_schedule_{SCHEDULE}.csv')
...
def plotWIBQL(ARMS, SCHEDULE, INTERVAL):
    ...
    for i in range(RUNS):
        wibqlFileName = (
            f'../testResults/deadline_env/wibqlResults_arms_{ARMS}_run_{i}_schedule_{SCHEDULE}.csv')
...
fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(WIDTH, HEIGHT),
                         gridspec_kw={'wspace': 0.13, 'hspace': 0.0}, frameon=False)
...
for i in range(len(ARMSLIST)):
    ...
    axes[i].hlines(xmin=0, xmax=EPISODEEND, y=deadlineRewards, label='Deadline Index', color='r',
                   linewidth=LINEWIDTH, linestyle='dashdot', zorder=4)
    axes[i].fill_between(x=numEpisode, y1=deadlinePercentile5, y2=deadlinePercentile95, alpha=0.2,
                         color='orange')
    axes[i].plot(numEpisode, NeurWINRewards, label='NeurWIN', linewidth=LINEWIDTH, linestyle='solid',
                 zorder=5)
    axes[i].fill_between(x=numEpisode, y1=NeurWINPercentile5, y2=NeurWINPercentile95, alpha=0.2,
                         color='green')
    axes[i].plot(numEpisode, aqlRewards, label=f'AQL', color='saddlebrown', linewidth=LINEWIDTH,
                 linestyle='dotted')

    axes[i].fill_between(x=numEpisode, y1=aqlPercentile5, y2=aqlPercentile95, alpha=0.2,
                         color='saddlebrown')
    axes[i].plot(numEpisode, qwicRewards, label=f'QWIC', color='g', linewidth=LINEWIDTH,
                 linestyle=(0, (3, 1, 1, 1, 1, 1)))
    axes[i].fill_between(x=numEpisode, y1=qwicPercentile5, y2=qwicPercentile95, alpha=0.2,
                         color='teal')
    axes[i].plot(numEpisode, wibqlRewards, label=f'WIBQL', color='lightseagreen',
                 linewidth=LINEWIDTH, linestyle=(0, (3, 1, 3, 3, 1, 3)))
    axes[i].fill_between(x=numEpisode, y1=wibqlPercentile5, y2=wibqlPercentile95, alpha=0.2,
                         color='lightseagreen')

    if ARMS == 100:
        pass
    else:
        reinforceRewards, reinforcePercentile5, reinforcePercentile95 = plotReinforce(ARMS, SCHEDULE,
                                                                                      INTERVAL)
        axes[i].plot(numEpisode, reinforceRewards, label=f'REINFORCE', color='k',
                     linewidth=LINEWIDTH, linestyle='dotted')
        axes[i].fill_between(x=numEpisode, y1=reinforcePercentile5, y2=reinforcePercentile95,
                             alpha=0.2, color='k')
        axes[i].plot(numEpisode, wolpRewards, label='WOLP-DDPG', color='darkorchid',
                     linewidth=LINEWIDTH, linestyle='dashed')
        axes[i].fill_between(x=numEpisode, y1=wolpPercentile5, y2=wolpPercentile95, alpha=0.2,
                             color='darkorchid')
