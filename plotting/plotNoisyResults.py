...
######################################## PLOTTING DEADLINE ##################################################################
INTERVAL = 20
numEpisode = np.arange(0, DEADLINEEPISODEEN D +INTERVAL, INTERVAL)

def plotDeadlineIndex(ARMS, SCHEDULE):

    deadlineFileName = \
        (f'../testResults/deadline_env/deadlineIndexResults_arms_{ARMS}_timeLimit_{TIMELIMIT}_schedule_{SCHEDULE}.csv')
...
deadlineIndex = axes[0].hlines(xmin=0, xmax=DEADLINEEPISODEEND, y=deadlineRewards,
                               label='Deadline Index', color='maroon', linewidth=LINEWIDTH,
                               linestyle='dashdot')
axes[0].fill_between(x=numEpisode, y1=deadlinePercentile5, y2=deadlinePercentile95, alpha=0.2,
                     color='orange')  # , label='Deadline Whittle Index confidence bound')
...
for var in NOISEVARS:
    ...
    for i in range(RUNS):
        if var == 0.0:
            NeurWINFileName = (
                f'../testResults/deadline_env/nnIndexResults_arms_{ARMS}_batchSize_{BATCHSIZE}_run_{i}_timeLimit_{TIMELIMIT}_schedule_{SCHEDULE}.csv')
        else:
            NeurWINFileName = (
                f'../testResults/deadline_env/noisy_results/noise_val_{var}/nnIndexResults_arms_{ARMS}_batchSize_{BATCHSIZE}_run_{i}_timeLimit_{TIMELIMIT}_schedule_{SCHEDULE}.csv')
    ...
    # print(x)
    axes[0].plot(numEpisode, nnVal, label=f'NeurWIN {np.sqrt(var) * 100:.0f}% error',
                 color=colors[x], linewidth=LINEWIDTH, linestyle=linestyles[x])
    axes[0].fill_between(x=numEpisode, y1=percentile5, y2=percentile95, alpha=0.2,
                         color=colors[x])  # , label='NeurWIN confidence bound')
...
d20LookAheadFileName = (
    f'../testResults/recovering_env/dLookAheadResults_arms_{ARMS}_timeLimit_{TIMELIMIT}_d_{20}_case_{CASE}_schedule_{SCHEDULE}.csv')
df = pd.read_csv(d20LookAheadFileName)
d20rewards = list(df.iloc[:, 1])

d20Lookahead = axes[1].hlines(xmin=0, xmax=RECOVERINGEPISODEEND, y=d20rewards,
                              label='Oracle, d = 20', color='orange', linewidth=LINEWIDTH,
                              linestyle='dashdot')
...
for var in NOISEVARS:
    if var == 0:
        neurwinFileName = (
            f'../testResults/recovering_env/nnIndexResults_arms_{ARMS}_batchSize_{BATCHSIZE}_case_{CASE}_timeLimit_{TIMELIMIT}_schedule_{SCHEDULE}.csv')
    else:
        neurwinFileName = (
            f'../testResults/recovering_env/noisy_results/noise_val_{var}/nnIndexResults_arms_{ARMS}_batchSize_{BATCHSIZE}_case_{CASE}_timeLimit_{TIMELIMIT}_schedule_{SCHEDULE}.csv')
    df = pd.read_csv(neurwinFileName)
    nnRewards = df.iloc[:, 1]
    axes[1].plot(numEpisode, nnRewards[:301], label=f'NeurWIN ${np.sqrt(var) * 100:.0f}$% error',
                 color=colors[x], linewidth=LINEWIDTH, linestyle=linestyles[x])
...
sizeAwareFileName = (
    f'../testResults/size_aware_env/case_{CASE}/sizeAwareIndexResults_arms_{ARMS}_schedule_{SCHEDULE}_arms.csv')
...
sizeAwareIndex = axes[2].hlines(xmin=0, xmax=WIRELESSEPISODEEND, y=sizeAwareRewards,
                                label='Size-aware Index', color='r', linewidth=LINEWIDTH,
                                linestyle='dashdot')
axes[2].fill_between(x=numEpisode, y1=sizeAware5Percentile, y2=sizeAware95Percentile, alpha=0.2,
                     color='orange')
...
for var in NOISEVARS:
    ...
    for i in range(RUNS):
        if var == 0.0:
            NeurWINFileName = (
                f'../testResults/size_aware_env/case_{CASE}/nnIndexResults_arms_{ARMS}_batchSize_{BATCHSIZE}_run_{i}_schedule_{SCHEDULE}_arms.csv')
        else:
            NeurWINFileName = (
                f'../testResults/size_aware_env/noisy_results/noise_val_{var}/case_{CASE}/nnIndexResults_arms_{ARMS}_batchSize_{BATCHSIZE}_run_{i}_schedule_{SCHEDULE}_arms.csv')
    ...
    axes[2].plot(numEpisode, nnVal[:31], label=f'NeurWIN {np.sqrt(var) * 100:.0f}% error',
                 color=colors[x], linewidth=LINEWIDTH, linestyle=linestyles[x])
    axes[2].fill_between(x=numEpisode, y1=percentile5[:31], y2=percentile95[:31], alpha=0.2,
                         color=colors[x])  # , label='NeurWIN Confidence Bound')
