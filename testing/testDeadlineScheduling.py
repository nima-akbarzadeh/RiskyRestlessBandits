...
def initialize():
    ...
    for i in range(ARMS):
        env = deadlineSchedulingEnv(seed=envSeeds[i], numEpisodes=numEpisodes,
                                    episodeLimit=TIMELIMIT, maxDeadline=12,
                                    maxLoad=9, newJobProb=jobProbs, processingCost=PROCESSINGCOST,
                                    train=False, batchSize=EPISODESEND, noiseVar=0)
        envs[i] = env
...
def selectDeadlineIndex():
    ...
    for key in states:
        if states[key][1] == 0:
            ...
            firstVal = (BETA ** (states[key][0] - 1)) * (
                        0.2 * (states[key][1] - states[key][0] + 1) ** 2)
            secondVal = (BETA ** (states[key][0] - 1)) * (
                        0.2 * (states[key][1] - states[key][0]) ** 2)
...
def initializeQLearningTraining():
    ...
    for i in range(ARMS):
        env = deadlineSchedulingEnv(seed=trainingEnvSeeds[i], numEpisodes=EPISODESEND,
                                    episodeLimit=TIMELIMIT, maxDeadline=12,
                                    maxLoad=9, newJobProb=jobProbs, processingCost=PROCESSINGCOST,
                                    train=True, batchSize=BATCHSIZE, noiseVar=0)
        trainingEnvs[i] = env
...
def createWibqlAgents():
    ...
    for key in trainingEnvs:
        agents[key] = WIBQL(numEpisodes=EPISODESEND, episodeLimit=TIMELIMIT, numArms=ARMS,
                            env=trainingEnvs[key], stateTable=stateArray)
...
def createWiqblAgentsTesting():
    global agents, trained_vals, envs, stateArray, envSeeds
    for key in envs:
        agents[key] = WIBQL(numEpisodes=numEpisodes, episodeLimit=TIMELIMIT, numArms=ARMS,
                            env=envs[key], stateTable=stateArray)
